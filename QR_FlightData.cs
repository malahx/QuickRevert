...
namespace QuickRevert {
	public class QFlightData : QuickRevert {
		...
		private static string FileFlightState = KSPUtil.ApplicationRootPath + "GameData/" + MOD + "/PluginData/{0}-flightstate.txt";

		private static string PathFlightState {
			get {
		...
		private static bool ConfigNodeHasPostInitState(ConfigNode nodes) {
		...
		private static bool isHardSaved {
			get {
				if (!HighLogic.LoadedSceneIsGame) {
					return false;
				}
		...
		private static bool CanStorePostInitState {
			get {
		...
		private static bool CanStorePreLaunchState {
			get {
				if (!HighLogic.LoadedSceneIsFlight) {
					return false;
				}
